...
class TodoManager {
  ...
  addTodo(task, dueDate, time, status = "pending") {
    const newTodo = {
      id: this.getRandomId(),
...
class UIManager {
  ...
  handleAddTodo() {
    ...
    if (task === "") {
      this.showAlertMessage("Masukkan kegiatanmu", "error");
    } else {
      const newTodo = this.todoManager.addTodo(task, dueDate, time);
      this.showAllTodos();
      this.taskInput.value = "";
      this.dateInput.value = "";
      this.timeInput.value = "";
      this.showAlertMessage("Kegiatanmu hari ini sudah dijadwalkan", "success");
    }
  }
  ...
  handleClearAllTodos() {
    ...
    this.showAllTodos();
    this.showAlertMessage("Semua kegiatan telah dihapus", "success");
  }
    ...
    try {
      ...
      if (Array.isArray(importedTodos)) {
        importedTodos.forEach(todo => {
          todoManager.addTodo(todo.task, todo.dueDate, todo.time, todo.status);
        });
        uiManager.showAllTodos();
        uiManager.showAlertMessage("Todos imported successfully", "success");
      } else {
        uiManager.showAlertMessage("Invalid file format", "error");
      }
    } catch (error) {
      uiManager.showAlertMessage("Failed to import todos", "error");
    }
  };
